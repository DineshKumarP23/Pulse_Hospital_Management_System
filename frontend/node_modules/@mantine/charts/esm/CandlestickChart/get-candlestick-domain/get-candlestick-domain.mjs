"use client";
//#region packages/@mantine/charts/src/CandlestickChart/get-candlestick-domain/get-candlestick-domain.ts
...
*/
function getCandlestickDomain({ data, openKey, highKey, lowKey, closeKey }) {
	...
	for (const item of data) {
		...
		if (![
			...
			low,
			item[closeKey]
		].every(isRenderableValue)) continue;
		if (low < dataMin) dataMin = low;
		if (high > dataMax) dataMax = high;
	}
	if (dataMin === Infinity || dataMax === -Infinity) return;
	const padding = (dataMax - dataMin) * .05 || 1;
	return [dataMin - padding, dataMax + padding];
}
//#endregion
export { getCandlestickDomain, isRenderableValue };

//# sourceMappingURL=get-candlestick-domain.mjs.map